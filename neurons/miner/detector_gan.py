        ...
        if modality == "image":
            ...
            with torch.no_grad():
                ...
                if isinstance(model_output, tuple) and len(model_output) >= 3:
                    ...
                    probs_list[-1], probs_list[-2] = probs_list[-2], probs_list[-1]
                    return probs_list
        ...
        with torch.no_grad():
            for frame in media_tensor:  # frame: [C, H, W]
                ...
                if isinstance(model_output, tuple) and len(model_output) >= 3:
                    ...
                    probs_list[-1], probs_list[-2] = probs_list[-2], probs_list[-1]
                    probs_tensor = torch.tensor(probs_list, device=probs.device, dtype=probs.dtype)
                    frame_probs.append(probs_tensor)
                elif isinstance(model_output, tuple) and len(model_output) == 2:
                    # Model without uncertainty estimation
                    logits, features = model_output
                    probs = F.softmax(logits, dim=1)
